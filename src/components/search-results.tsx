...
export function SearchResults({
  ...
  const filteredGuides = React.useMemo(
    () => guides.filter((g) => g.location === normalizedDestination),
    [normalizedDestination]
  )

  const filteredHotels = React.useMemo(
    () => hotels.filter((h) => h.location === normalizedDestination),
    [normalizedDestination]
  )

  const filteredFood = React.useMemo(
    () => food.filter((f) => f.location === normalizedDestination),
    [normalizedDestination]
  )

  const filteredHospitals = React.useMemo(
    () => hospitals.filter((h) => h.location === normalizedDestination),
    [normalizedDestination]
  )

  const filteredPlaces = React.useMemo(
    () => touristPlaces.filter((p) => p.location === normalizedDestination),
    [normalizedDestination]
  )
    ...
    <div className="w-full">
        ...
        <TabsContent value="hospitals" className="space-y-4">
          {filteredHospitals.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              No hospitals found for {normalizedDestination}.
            </div>
          ) : (
            filteredHospitals.map((h) => (
              <Card key={h.id}>
                ...
                <CardHeader className="p-4 pb-2">
                  ...
                  <CardDescription className="flex items-center gap-1">
                    <MapPin className="h-3 w-3" /> {h.distance} away
                  </CardDescription>
                </CardHeader>
                ...
                <CardContent className="p-4 pt-0 space-y-2">
                  ...
                  {h.doctors.map((d) => (
                    <div
                      key={d.id}
                      className="flex items-center gap-3 text-sm border p-2 rounded"
                    >
                ...
                <CardFooter className="p-4 pt-0 gap-2">
                  <a
                    href={`tel:${h.contact}`}
                    className="flex-1 inline-flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 text-sm hover:bg-accent hover:text-accent-foreground"
                  >
                  ...
                  <Button
                    className="flex-1"
                    onClick={() =>
                      onAddToItinerary(h.name, "Hospital", 0)
                    }
                  >
                    Save
                  </Button>
      ...
      {selectedGuideDetails && (
        <GuideDetailsDialog
          ...
          onOpenChange={(o) => !o && setSelectedGuideDetails(null)}
          onStartChat={() =>
            setSelectedGuide(selectedGuideDetails.name)
          }
        />
      )}
    </div>
